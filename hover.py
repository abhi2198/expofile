...
class OffbPosCtl:
	...
	if len(msgs.detections)>0:
	 ...
	 #   print(self.tag_pt_x,self.tag_pt_y)
	...
	des_po = PoseStamped()
	des_po.header.frame_id = "world"
	des_po.header.stamp=rospy.Time.from_sec(time.time())
        #des_po.coordinate_frame= 8
	#des_po.type_mask = 3527
	des_po.pose.position.x = x
	des_po.pose.position.y = y
	des_po.pose.position.z = z
	return des_po
	...
	tag_fixed_x = self.tag_pt_x
        ...
        while self.mode == "SWOOP" and self.curr_pose.pose.position.z > 0.002 and not rospy.is_shutdown():
	    ...
	    err_x = des_x - curr_x
	    err_y = des_y - curr_y 
	    x_change = -((err_x*self.KP) - (((err_x - self.x_prev_error)*1100)* self.KD)) #+ (self.x_sum_error * self.KI*0.1))
            y_change = -((err_y*self.KP) - (((err_y - self.y_prev_error)*1100)* self.KD)) #+ (self.y_sum_error * self.KI*0.1))	
